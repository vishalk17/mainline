...
{
	/* pr_err ("@@ timeOld(%d, %d), timeNew(%d, %d)", */
...
{
	pr_err("[VCODEC][MMDVFS_VDEC] VdecDVFS monitor %dms, decoded %d frames\n",
		 MONITOR_DURATION_MS,
		 gMMDFVFSMonitorCounts);
	pr_err("[VCODEC][MMDVFS_VDEC] total time %d, max duration %d, target lv %d\n",
		 gHWLockInterval,
		 gHWLockMaxDuration,
		 level);
...
void VdecDvfsAdjustment(void)
{
	...
	if (gMMDFVFSMonitorStarts == VAL_TRUE && gMMDFVFSMonitorCounts > MONITOR_START_MINUS_1) {
		...
		if (_monitor_duration < MONITOR_DURATION_MS) {
			...
			pr_err("[VCODEC][MMDVFS_VDEC] DROP_PERCENTAGE = %d, RAISE_PERCENTAGE = %d\n",
				 DROP_PERCENTAGE, RAISE_PERCENTAGE);
			pr_err("[VCODEC][MMDVFS_VDEC] reset monitor duration (%d ms), percent: %d\n",
	...
	if (gMMDFVFSMonitorStarts == VAL_TRUE) {
		...
		if (gMMDFVFSMonitorCounts > MONITOR_START_MINUS_1) {
			if (gFirstDvfsLock == VAL_TRUE) {
				...
				/* pr_err("[VCODEC][MMDVFS_VDEC] LOCK 1 start monitor instance = 0x%p\n", */
...
void vdec_power_on(void)
{
	...
	if (ret) {
		...
		pr_err("[VCODEC][ERROR][vdec_power_on] clk_MT_SCP_SYS_MM0 is not enabled, ret = %d\n", ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_SMI_COMMON is not enabled, ret = %d\n",
		ret);
		}
	...
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_GALS_VDEC2MM is not enabled, ret = %d\n",
		ret);
		}
#endif
	...
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][vdec_power_on] clk_MT_SCP_SYS_VDE is not enabled, ret = %d\n", ret);
	}

	ret = clk_prepare_enable(clk_MT_CG_VDEC);
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_VDEC is not enabled, ret = %d\n", ret);
	}
...
void venc_power_on(void)
{
	...
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][venc_power_on] clk_MT_SCP_SYS_MM0 is not enabled, ret = %d\n", ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][venc_power_on] clk_MT_CG_SMI_COMMON is not enabled, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][venc_power_on] clk_MT_CG_GALS_VENC2MM is not enabled, ret = %d\n",
		ret);
	}
#endif
	...
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][venc_power_on] clk_MT_SCP_SYS_VEN is not enabled, ret = %d\n", ret);
	}

	ret = clk_prepare_enable(clk_MT_CG_VENC_VENC);
	if (ret) {
		/* print error log & error handling */
		pr_err("[VCODEC][ERROR][venc_power_on] clk_MT_CG_VENC_VENC is not enabled, ret = %d\n", ret);
	}
...
void dec_isr(void)
{
	...
	if ((u4CgStatus & 0x10) != 0) {
		pr_err("[VCODEC][ERROR] DEC ISR, VDEC active is not 0x0 (0x%08x)", u4CgStatus);
		return;
	}

	u4DecDoneStatus = VDO_HW_READ(KVA_VDEC_MISC_BASE+0xA4);
	if ((u4DecDoneStatus & (0x1 << 16)) != 0x10000) {
		pr_err("[VCODEC][ERROR] DEC ISR, Decode done status is not 0x1 (0x%08x)", u4DecDoneStatus);
		return;
	}
...
static long vcodec_alloc_non_cache_buffer(unsigned long arg)
{
	...
	pr_err("VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n", current->pid);
...
static long vcodec_free_non_cache_buffer(unsigned long arg)
{
	...
	pr_err("VCODEC_FREE_NON_CACHE_BUFFER + tid = %d\n", current->pid);
...
{
	pr_err("[ERROR] VCODEC_LOCKHW, DecHWLockEvent TimeOut, CurrentTID = %d\n", current->pid);
	if (FirstUseDecHW != 1) {
...
{
	pr_err("[ERROR] VCODEC_LOCKHW EncHWLockEvent TimeOut, CurrentTID = %d\n", current->pid);
	...
	if (FirstUseEncHW != 1) {
		...
		if (grVcodecEncHWLock.pvHandle == 0) {
			...
			if (gLockTimeOutCount > 30) {
				pr_err("[ERROR] VCODEC_LOCKHW - ID %d fail\n", current->pid);
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (DecHWLockEvent.u4TimeoutMs == 1) {
				pr_err("VCODEC_LOCKHW, First Use Dec HW!!\n");
				FirstUseDecHW = 1;
			} else {
				FirstUseDecHW = 0;
			}
			...
			if (grVcodecDecHWLock.pvHandle ==
				...
				pr_err("may cause lock HW timeout!! instance = 0x%lx, CurrentTID = %d\n",
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (EncHWLockEvent.u4TimeoutMs == 1) {
				pr_err("VCODEC_LOCKHW, First Use Enc HW %d!!\n", rHWLock.eDriverType);
				FirstUseEncHW = 1;
			} else {
				FirstUseEncHW = 0;
			}
			...
			if (grVcodecEncHWLock.pvHandle ==
				...
				pr_err("may cause lock HW timeout!! instance=0x%lx, CurrentTID=%d, type:%d\n",
			...
			if (grVcodecEncHWLock.pvHandle == 0) { /* No process use HW, so current process can use HW */
				...
				if (gLockTimeOutCount > 30) {
					pr_err("[ERROR] VCODEC_LOCKHW %d fail,someone locked HW over 30 times\n",
						 current->pid);
					pr_err("without timeout 0x%lx,%lx,0x%lx,type:%d\n",
						 (VAL_ULONG_T)grVcodecEncHWLock.pvHandle,
						 pmem_user_v2p_video((VAL_ULONG_T)rHWLock.pvHandle),
						 (VAL_ULONG_T)rHWLock.pvHandle,
						 rHWLock.eDriverType);
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{
		/* pr_err("VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid); */
		/* pr_err("VCODEC_SET_THREAD_ID [EMPTY] - tid = %d\n", current->pid); */
	}
	break;
	...
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
	{
		...
		if (ret) {
			pr_err("[ERROR] VCODEC_ALLOC_NON_CACHE_BUFFER failed! %lu\n", ret);
			return ret;
		}
	}
	break;
	...
	case VCODEC_FREE_NON_CACHE_BUFFER:
	{
		...
		if (ret) {
			pr_err("[ERROR] VCODEC_FREE_NON_CACHE_BUFFER failed! %lu\n", ret);
			return ret;
		}
	}
	break;
...
#ifdef ENABLE_MMDVFS_VDEC
		/* MM DVFS related */
		/* pr_err("[VCODEC][MMDVFS_VDEC] INC_DEC_EMI MM DVFS init\n"); */
		/* raise voltage */
	...
	case VCODEC_INC_PWR_USER:
	{
		...
		if (rPowerParam.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {
			...
			if (gu4L2CCounter == 1) {
				...
				pr_err("[VCODEC] Switch L2C size to 512K successful\n");
	...
	case VCODEC_DEC_PWR_USER:
	{
		...
		if (rPowerParam.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {
			...
			if (gu4L2CCounter == 0) {
				...
				pr_err("[VCODEC] Switch L2C size to 0K successful\n");
			}
		}
#endif
		mutex_unlock(&L2CLock);
	...
	{
		pr_err("VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
		pr_err("VCODEC_INITHWLOCK [EMPTY] - - tid = %d\n", current->pid);
	}
	break;
	...
	{
		pr_err("VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
		pr_err("VCODEC_DEINITHWLOCK [EMPTY] - - tid = %d\n", current->pid);
	}
	break;
	...
	case VCODEC_GET_CORE_LOADING:
	{
		...
		if (rTempCoreLoading.CPUid < 0) {
			pr_err("[ERROR] rTempCoreLoading.CPUid(%d) < 0\n", rTempCoreLoading.CPUid);
			return -EFAULT;
		}
	...
	{
		pr_err("VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n", current->pid);
		user_data_addr = (VAL_UINT8_T *)arg;
		...
		pr_err("+VCODEC_SET_CPU_OPP_LIMIT (%d, %d, %d), tid = %d\n",
		 ...
		 * }
		 */
		pr_err("-VCODEC_SET_CPU_OPP_LIMIT tid = %d, ret = %lu\n", current->pid, ret);
		pr_err("VCODEC_SET_CPU_OPP_LIMIT [EMPTY] - - tid = %d\n", current->pid);
	}
	break;
...
static int vcodec_release(struct inode *inode, struct file *file)
{
	...
	mutex_lock(&DriverOpenCountLock);
	pr_err("vcodec_release pid = %d, Driver_Open_Count %d\n", current->pid, Driver_Open_Count);
	Driver_Open_Count--;
	...
	if (Driver_Open_Count == 0) {
		...
		mutex_lock(&L2CLock);
		if (gu4L2CCounter != 0) {
			pr_err("vcodec_flush pid = %d, L2 user = %d, force restore L2 settings\n",
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (IS_ERR(clk_MT_CG_SMI_COMMON)) {
		pr_err("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_SMI_COMMON\n");
	...
	if (IS_ERR(clk_MT_CG_GALS_VDEC2MM)) {
		pr_err("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_GALS_VDEC2MM\n");
	...
	if (IS_ERR(clk_MT_CG_GALS_VENC2MM)) {
		pr_err("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_GALS_VENC2MM\n");
	...
	if (IS_ERR(clk_MT_CG_VENC_VENC)) {
		pr_err("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VENC_VENC\n");
	...
	if (IS_ERR(clk_MT_SCP_SYS_MM0)) {
		pr_err("[VCODEC][ERROR] Unable to devm_clk_get MT_SCP_SYS_MM0\n");
...
static int __init vcodec_driver_init(void)
{
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
		pr_err("[VCODEC][ERROR] create dec hwlock event error\n");
	}
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
		pr_err("[VCODEC][ERROR] create enc hwlock event error\n");
	}
...
static void __exit vcodec_driver_exit(void)
{
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
		pr_err("[VCODEC][ERROR] close dec hwlock event error\n");
	}

	eValHWLockRet = eVideoCloseEvent(&EncHWLockEvent, sizeof(VAL_EVENT_T));
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
		pr_err("[VCODEC][ERROR] close enc hwlock event error\n");
	}
